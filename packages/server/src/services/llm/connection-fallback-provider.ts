...
export function withConnectionFallbackProvider({
  ...
  // A background admission's `beforeAttempt` books one logical scheduled
  // attempt. Retrying the whole primary/fallback chain would invoke that hook
  // again, incorrectly consuming quota for the same scheduled generation.
  // Keep transport retries on the individual provider requests inside their
  // admission wrappers instead, so the booking spans all transient attempts.
  const preserveBackgroundClaim = admissionMode.kind === "background";
  const admittedPrimary = withConnectionAdmissionProvider(
    preserveBackgroundClaim ? primary : unwrapLlmTransportRetries(primary),
    primaryConnectionId,
    primaryMode,
  );
  const fallback = withConnectionAdmissionProvider(
    preserveBackgroundClaim ? fallbackProvider : unwrapLlmTransportRetries(fallbackProvider),
  ...
  return preserveBackgroundClaim ? chained : withLlmTransportRetries(chained);
}
