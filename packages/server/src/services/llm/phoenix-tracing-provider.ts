...
export function isPhoenixLlmTracingEnabled(): boolean {
  if (isEnabled(process.env.PHOENIX_LLM_TRACING_ENABLED)) return true;
  const endpoint = process.env.PHOENIX_COLLECTOR_ENDPOINT?.trim();
  const apiKey = process.env.PHOENIX_API_KEY?.trim();
  // A non-default collector or an API key is an explicit opt-in for hosted
  // Phoenix. The example's local default endpoint alone remains disabled.
  return Boolean(apiKey || (endpoint && endpoint !== DEFAULT_COLLECTOR_ENDPOINT));
}
...
function getPhoenixTracer(): ReturnType<typeof trace.getTracer> | null {
  ...
  try {
    ...
    register({ projectName, url: endpoint, apiKey: process.env.PHOENIX_API_KEY?.trim() || undefined, batch: false });
