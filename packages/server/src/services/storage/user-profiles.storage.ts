...
export function createUserProfilesStorage(db: DB) {
  return {
    ...
    async migrateLegacyClientState(id: string, data: PatchUserProfileContinuityInput) {
      ...
      await db
        .update(userProfiles)
        .set({ legacyClientStateMigrated: "true", updatedAt: now() })
        .where(eq(userProfiles.id, id));
      return { row: await this.getById(id) ?? row, migrated: true };
    },
    async clearPersonaReferences(personaId: string) {
      const profiles = await this.list();
      await Promise.all(
        profiles.filter((profile) => profile.activePersonaId === personaId).map((profile) => this.update(profile.id, { activePersonaId: null })),
      );
    },
  };
}
