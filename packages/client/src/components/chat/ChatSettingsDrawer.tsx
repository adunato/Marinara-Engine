  ...
  const updateDailyMemoryAgentConfig = useCallback(
    async ({ settings, connectionId }: { settings?: Record<string, unknown>; connectionId?: string | null }) => {
      if (!dailyMemoryAgentConfig) {
        toast.error(localizeUi("ui.chat.chatsettingsdrawer.dailyConversationMemoriesConfigurationIsUnavailable"));
        return;
      }
      ...
      try {
        ...
        toast.error(
          error instanceof Error
            ? error.message
            : localizeUi("ui.chat.chatsettingsdrawer.couldNotUpdateDailyMemoriesSettings"),
        );
      }
    },
    [dailyMemoryAgentConfig, localizeUi, updateAgentConfig],
  );
    ...
    return (
      <AgentCategorySection label={label} icon={icon} description={description} count={activeAgents.length}>
        ...
        {inactiveAgents.length > 0 ? (
          ...
          <p className="px-1 text-[0.625rem] text-[var(--muted-foreground)]">
            {localizeUi("ui.chat.chatsettingsdrawer.allConversation")} {label.toLowerCase()}{" "}
            {localizeUi("ui.chat.chatsettingsdrawer.areActive")}
          </p>
        )}
      </AgentCategorySection>
    );
  ...
  const renderDailyMemoryAgentSettings = () => {
    ...
    return (
      <AgentSettingsCard
        ...
        title={localizeUi("ui.chat.chatsettingsdrawer.dailyConversationMemories")}
        description={localizeUi("ui.chat.chatsettingsdrawer.configureFormationAndRetrievalTheseAgentDefaultsAreShared")}
        badge={
          updateAgentConfig.isPending ? (
            <span className="inline-flex items-center gap-1 text-[0.5625rem] text-[var(--muted-foreground)]">
              <Loader2 size="0.625rem" className="animate-spin" /> {localizeUi("ui.noodle.noodlehome.saving")}
            </span>
          ) : null
        }
      >
        <div className="grid gap-2 sm:grid-cols-2">
          <label className="space-y-1 text-[0.625rem] text-[var(--muted-foreground)]">
            <span className="font-medium">{localizeUi("ui.chat.chatsettingsdrawer.memoryFormationConnection")}</span>
            <select
              aria-label={localizeUi("ui.chat.chatsettingsdrawer.dailyMemoryFormationConnection")}
            ...
            >
              <option value="">{localizeUi("ui.chat.chatsettingsdrawer.agentDefaultThenChatConnection")}</option>
          ...
          <label className="space-y-1 text-[0.625rem] text-[var(--muted-foreground)]">
            <span className="font-medium">{localizeUi("ui.chat.chatsettingsdrawer.dailyHandoverHour")}</span>
            <select
              aria-label={localizeUi("ui.chat.chatsettingsdrawer.conversationDailyMemoryHandoverTime")}
          ...
          <label className="space-y-1 text-[0.625rem] text-[var(--muted-foreground)]">
            <span className="font-medium">{localizeUi("ui.chat.chatsettingsdrawer.recentMessagesForRetrieval")}</span>
            <DraftNumberInput
            ...
            {(
              ...
              <label key={key} className="space-y-1 text-center text-[0.5625rem] text-[var(--muted-foreground)]">
                <span
                  ...
                  title={localizeUi("ui.chat.chatsettingsdrawer.value1Weight", { value1: label })}
                >
                  {label}
                </span>
                <DraftNumberInput
        ...
        <label className="space-y-1.5 text-[0.625rem] text-[var(--muted-foreground)]">
          <span className="flex items-center justify-between gap-2 font-medium">
            <span>{localizeUi("ui.chat.chatsettingsdrawer.minimumRank")}</span>
          ...
          <span className="block text-[0.5625rem] leading-relaxed">
            {localizeUi("ui.chat.chatsettingsdrawer.memoriesRankedBelowThisPercentageAreExcludedFromPreview")}
          </span>
        </label>
        ...
        >
          <Eye size="0.75rem" /> {localizeUi("ui.chat.chatsettingsdrawer.previewCurrentMemoryExtraction")}
        </button>
        <button
          type="button"
          onClick={() => {
        ...
        >
          <Settings2 size="0.75rem" /> {localizeUi("ui.chat.chatsettingsdrawer.openFullAgentSettingsAndPrompt")}
        </button>
      </AgentSettingsCard>
    );
  };
    ...
    return (
      <AgentSettingsCard
        ...
        description={localizeUi(
          "ui.chat.chatsettingsdrawer.configureFourConversationScopedIntentionAreasAndManualOffline",
        )}
      >
        {!eligible && (
          <p className="rounded-lg bg-amber-400/10 px-3 py-2 text-[0.625rem] leading-relaxed text-amber-300">
            {localizeUi(
              "ui.chat.chatsettingsdrawer.dailyIntentionsCurrentlySupportsConversationsWithExactlyOneCharacter",
            )}
          </p>
        )}
        <p className="text-[0.625rem] leading-relaxed text-[var(--muted-foreground)]">
          {localizeUi("ui.chat.chatsettingsdrawer.theCutoffIsInformationalInThisManualOnlyFirst")}
        </p>
        <button
          type="button"
        ...
        >
          <Settings2 size="0.75rem" /> {localizeUi("ui.chat.chatsettingsdrawer.configureIntentionAreas")}
        </button>
      </AgentSettingsCard>
    );
  };
  ...
  const renderCharacterMindAgentSettings = () => {
    ...
    return (
      <AgentSettingsCard
        ...
        description={localizeUi("ui.chat.chatsettingsdrawer.buildAndOperateEachCharacterSMarkdownWikiFrom")}
      >
        <p className="text-[0.625rem] leading-relaxed text-[var(--muted-foreground)]">
          {localizeUi("ui.chat.chatsettingsdrawer.configureTheSharedConnectionInspectLiveStatusBuildOr")}
        </p>
        <button
          type="button"
          ...
          className="inline-flex items-center justify-center gap-1.5 rounded-lg border border-[var(--border)] bg-[var(--background)] px-3 py-2 text-xs font-medium text-[var(--foreground)] transition-colors hover:bg-[var(--accent)]"
        >
          <Settings2 size="0.75rem" /> {localizeUi("ui.chat.chatsettingsdrawer.manageCharacterMind")}
        </button>
      </AgentSettingsCard>
    );
  };
        ...
        >
          ...
          {chatMode === "roleplay" && (
            <Section
              ...
              label={localizeUi("ui.chat.chatsettingsdrawer.sourceChats")}
              icon={<ArrowRightLeft size="0.875rem" />}
              help={localizeUi(
                "ui.chat.chatsettingsdrawer.useSummariesAndRecentMessagesFromExistingConversationRoleplay",
              )}
            >
          ...
          {isConversation && metadata.enableAgents === true && activeAgentIds.includes(DAILY_MEMORY_AGENT_ID) && (
            <Section
              label={localizeUi("ui.chat.chatsettingsdrawer.dailyMemories")}
              icon={<Brain size="0.875rem" />}
              help={localizeUi("ui.chat.chatsettingsdrawer.reviewTheMemoriesFormedForEachCompletedConversationDay")}
            >
              <button
                type="button"
              ...
              >
                <div className="min-w-0 flex-1">
                  <span className="text-[0.6875rem] font-medium">
                    {localizeUi("ui.chat.chatsettingsdrawer.editDailyMemories")}
                  </span>
                  <p className="text-[0.625rem] text-[var(--muted-foreground)]">
                    {localizeUi("ui.chat.chatsettingsdrawer.memoriesAreGroupedByCompletedDayAndRemainSeparate")}
                  </p>
                </div>
                <Pencil size="0.875rem" className="shrink-0 text-[var(--muted-foreground)]" />
              </button>
          ...
          {dailyIntentionsAgentActive && (
            <Section
              label={localizeUi("ui.chat.chatsettingsdrawer.dailyIntentions")}
              icon={<Brain size="0.875rem" />}
              help={localizeUi("ui.chat.chatsettingsdrawer.reviewEditOrManuallyRegenerateTheSingleCharacterS")}
            >
              <button
                type="button"
              ...
              >
                <div className="min-w-0 flex-1">
                  <span className="text-[0.6875rem] font-medium">
                    {localizeUi("ui.chat.chatsettingsdrawer.editDailyIntentions")}
                  </span>
                  <p className="text-[0.625rem] text-[var(--muted-foreground)]">
                    {localizeUi("ui.chat.chatsettingsdrawer.runAllEnabledAreasSequentiallyRerunOneAreaOr")}
                  </p>
                </div>
                <Pencil size="0.875rem" className="shrink-0 text-[var(--muted-foreground)]" />
              </button>
        ...
        {agentAddPreview && (
          <div className="space-y-4">
            ...
            {agentAddIsFeature ? (
              ...
              <div className="rounded-xl bg-[var(--secondary)]/70 px-3 py-2.5 text-[0.6875rem] leading-5 text-[var(--muted-foreground)] ring-1 ring-[var(--border)]">
                {localizeUi("ui.chat.chatsettingsdrawer.dailyIntentionsUsesThePreceding24HoursOfVisible")}
              </div>
            ) : (
              <div className="space-y-1.5">
