...
function formatRegressionAwarenessConversation(
  chatName: string,
  wrapFormat: "xml" | "markdown" | "none",
  message?: string,
): string {
  return formatAwarenessConversation({
    chatName,
    memberNames: ["Mari", "Rana"],
    personaName: "Mari",
    characterNames: new Map([["char-rana", "Rana"]]),
    metadata: {},
    messages: message
      ? [
          {
...
const cases: RegressionCase[] = [
  {
    name: "character emotion profiles normalize safely and resolve per character",
    run() {
      const profile = normalizeCharacterEmotionProfile({
        enabled: true,
        defaultStateId: " Calm State ",
        states: [
          { id: "Calm_State", label: "Calm", description: "Settled and attentive.", spriteExpression: " neutral " },
          { id: "happy", label: "Happy", description: "Warmly pleased and playful.", spriteExpression: "" },
        ],
      });
      assert.deepEqual(profile, {
        ...
        defaultStateId: "calm-state",
        states: [
          { id: "calm-state", label: "Calm", description: "Settled and attentive.", spriteExpression: "neutral" },
          { id: "happy", label: "Happy", description: "Warmly pleased and playful.", spriteExpression: null },
        ],
      });
      assert.equal(
        normalizeCharacterEmotionProfile({
      ...
      const baseContext = { user: "Mari", char: "Narrator", characters: ["Kate", "Noor"], variables: {} };
      const conditional = '{{#if charEmotion == "happy"}}warm{{else if charEmotion == "angry"}}terse{{else}}steady{{/if}}';
...
{{char}}: {{charEmotion}} {{#if charEmotion == "happy"}}warm{{else}}steady{{/if}}
        ...
        { ...baseContext, characterProfiles: [{ name: "Kate", emotion: "happy" }, { name: "Noor", emotion: "angry" }] },
      );
  ...
  },
  {
    ...
    run() {
      for (const wrapFormat of ["xml", "markdown", "none"] as const) {
        const conversation = formatRegressionAwarenessConversation(
          "Another Lab",
          wrapFormat,
        ...
        assert.match(awareness, /Conversation name: Another Lab/);
        assert.match(awareness, /Rana: The experiment is stable\./);
        if (wrapFormat === "xml") {
          assert.match(awareness, /^<cross_chat_awareness>/);
          assert.match(awareness, /<source_conversation>/);
        } else if (wrapFormat === "markdown") {
          assert.match(awareness, /^## Cross Chat Awareness/);
          assert.match(awareness, /^### Source Conversation/m);
          assert.equal(awareness.includes("<cross_chat_awareness>"), false);
        } else {
          assert.equal(awareness.includes("<cross_chat_awareness>"), false);
          assert.equal(awareness.includes("## Cross Chat Awareness"), false);
          assert.equal(awareness.includes("### Source Conversation"), false);
        }
      }
    },
  },
  {
    ...
    async run() {
      ...
      const xmlAwareness = formatAwarenessContextBlock(
        [formatRegressionAwarenessConversation("Existing XML awareness.", "xml")],
        "xml",
      );
      const markdownAwareness = formatAwarenessContextBlock(
        [formatRegressionAwarenessConversation("Existing Markdown awareness.", "markdown")],
        "markdown",
      );
      const unwrappedAwareness = formatAwarenessContextBlock(
        [formatRegressionAwarenessConversation("Existing unwrapped awareness.", "none")],
        "none",
      );
      ...
      assert.match(xmlMerged, /^<cross_chat_awareness>/);
      ...
      assert.ok(xmlMerged.indexOf("<memories>") < xmlMerged.lastIndexOf("</cross_chat_awareness>"));
      assert.ok(markdownMerged);
      assert.match(markdownMerged, /Existing Markdown awareness\./);
      assert.match(markdownMerged, /^## Cross Chat Awareness/);
      assert.match(markdownMerged, /^### Memories/m);
      assert.match(markdownMerged, /^\\# forged heading/m);
      assert.equal(markdownMerged.includes("<cross_chat_awareness>"), false);
      ...
      assert.equal(unwrappedMerged.includes("<cross_chat_awareness>"), false);
      assert.equal(unwrappedMerged.includes("## Cross Chat Awareness"), false);
