...
const cases: RegressionCase[] = [
  {
    name: "two-pass Conversation reuses one source snapshot and isolates the writer",
    run() {
      const sourceScaffold = "STANDARD_RESPONSE_TEMPLATE_MUST_NOT_LEAK";
      const preparedStandardInput = [
        {
      ...
      const sourceSnapshot = createConversationSourceSnapshot(preparedStandardInput, sourceScaffold);
      ...
      for (const marker of [
        ...
        assert.match(sourceText, new RegExp(marker));
      }
      assert.doesNotMatch(sourceText, /STANDARD_RESPONSE_TEMPLATE_MUST_NOT_LEAK/u);

      preparedStandardInput[1]!.content = "MUTATED_AFTER_SNAPSHOT";
      ...
      assert.match(curatorMessages[1]!.content, /TRANSCRIPT_SECRET_19ca/u);
      assert.match(curatorMessages[1]!.content, /PREGEN_AGENT_SECRET_8d40/u);
      ...
      assert.doesNotMatch(writerText, /CHARACTER_CARD_SECRET_7f1d/u);
      assert.doesNotMatch(writerText, /DAILY_MEMORY_SECRET_b84e/u);
      assert.doesNotMatch(writerText, /TRANSCRIPT_SECRET_19ca/u);
      assert.doesNotMatch(writerText, /LORE_SECRET_673e/u);
      assert.doesNotMatch(writerText, /PREGEN_AGENT_SECRET_8d40/u);
    },
  },
  {
