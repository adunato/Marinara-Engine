...
| `MARINARA_ANDROID_SECRET`        | empty                                          | Internal local-auth secret for APK-managed Termux installs. The Android wrapper provisions it and the Termux launcher exports it; do not set it for ordinary desktop or manual Termux installs. When configured, it must be exactly 64 hexadecimal characters. An invalid non-empty value makes device-local requests fail with HTTP 503 rather than weakening authentication.                                                                                                      |
| `MARINARA_ANDROID_SECRET_FILE`   | `~/.marinara-engine/android-secret`            | Private secret-file path used by the Termux launcher and the local `mari` CLI. The APK and launcher manage this file automatically.                                                                                                                                                                                                                                                                                                                                              |
